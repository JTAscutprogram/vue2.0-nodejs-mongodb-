                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editCart(0,item)">-</a>
...
<script>
    ...
    import {currency} from './../util/currency';  //导入价格格式化文件

    export default{
        data(){
            return{
                cartList:[],            //用于缓存用户商品列表
                productId:'',           //用于缓存删除的商品ID
               // checkAllFlag:false,           //用于展示全选功能控制
                //totalPrice:0,           //用于实时显示所有商品价格
                modalConfirm:false      //用于控制Modal模态框的显示
          ...
          checkOut(){   //checkOut按钮绑定地址列表跳转函数
            ...
            init(){
                axios.get("/users/cartList").then((res)=>{
                    ...
                    this.cartList = result.result;     //将获取到的商品列表赋予对象
                  
                })
               /* if(this.cartList){
                  this.totalPrice = this.cartList.reduce(function(prev,cur,index,array){

                  })
                }*/
                
            },
            //用于与modal模态框子组件通信，控制模态框显示属性
            closeModal(){
            ...
            //删除某条商品数据数据
            delCart(){
                axios.post("/users/cartDelete",{
                    productId:this.productId
                }).then((res)=>{
                    let result = res.data;
                    if(result.status == 0){    //如果删除成功，重新渲染页面
                        this.modalConfirm =false;
                        this.init();        //重新渲染
                    }
                })
            ...
            //实现商品数字编辑
            editCart(NumOfCalculate,item){
                if(NumOfCalculate == 0){     //如果传进来是0做减法
                        if(item.productNum <= 1){   //不允许数据小于1
                            return
                        }
                        item.productNum--
                }
                else if (NumOfCalculate == 1){
                    item.productNum++
                } 
                else{           //表示点击选中
            ...
            //实现全选切换功能
            toggleCheckALl(){
                var flag= !this.checkAllFlag; //先切换全选的按钮状态
                this.cartList.forEach((item)=>{
                  item.checked = flag;   //然后切换所有商品的选中状态
                })
                axios.post("/users/editCheckAll",{
                  checkAll:flag
                }).then((res)=>{
                  let result = res.data;
                  if(result.status == 0){ //修改全选成功
                        console.log('checkAll suc')
                  }
                })
            }
            //
