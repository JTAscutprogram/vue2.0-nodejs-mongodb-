...
<div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="next-btn-wrap">
              ...
              :to ="{path:'orderConfirm',query:{'addressId':selectAddressId}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
    ...
    <modal :mdShow="isMdShow" @close="closeModal">  
        <p slot = "message">confirm to delete address?</p>
        <div slot = "btnGroup">
            <a href="javascript:;" class="btn btn--m" @click="delAddress">confirm</a>
...
//导入样式
  ...
  export default{
      data(){       //data中必须是闭包 返回局部变量，这样才不会影响其他组件
          return{
              checkIndex:0,             //绑定选中某条地址为默认地址
              //expandText:"more",            //展开栏文本
              limit:3,                  //控制默认显示地址数量 比如默认最多显示三条
              addressList:[],            //定义地址列表数组
              isMdShow:false  ,          //控制弹框
              addressId:''  ,               //删除地址需要全局保存地址
              selectAddressId:''          //保存用户需要送货选中的地址ID
      ...
      methods:{
            init(){     //地址初始化函数
                axios.get("/users/addressList").then((res)=>{   //这里的axios不用提供参数，因为是从cookies中取
                    let result = res.data;
            ...
            setDefault(addressId){       //设置默认地址的函数
                axios.post("/users/setDefault",{
                    addressId:addressId
                }).then((res)=>{
                    let result = res.data;
                    if (result.status == 0){
                        console.log('set Default succ')
                        this.init();
                    }
                })
            ...
            delAddress(){       //删除地址功能实现
                 ...
                 axios.post("/users/deleteAddress",{
                    ...
                    let result = res.data;
                    if (result.status == 0){
                        console.log('delete Default succ')
                        //数据变更之后重新渲染一次
                        this.init();
                    }
                })
